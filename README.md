...
**Полёт** — это окно длиной `term_months`, начинающееся с первого активного месяца.
...
**Цепочки переименований:** если проект менял ID (310 → 311), платежи считаются совместно, в `project_ids` пишется `"310|311"`, в `client_id` — актуальный (`311`).

---

## Известные ограничения

Полный список — в `AUDIT.md`. Кратко:

- **`report.csv` не эталон значений** — только образец формата. Расхождения с ним могут быть корректными.
- **Роль `service_changes.csv` не подтверждена** заказчиком. Мы используем смены услуг для разрыва полётов.
- **Порог «неизвестно»** — сейчас 3 месяца. Уточняется.
- **Пересечение оплат 320/321** — суммируется. Это допущение.
- **`label = end`** в текущих данных не встречается. Правило заложено, но не проверено на реальных данных.
